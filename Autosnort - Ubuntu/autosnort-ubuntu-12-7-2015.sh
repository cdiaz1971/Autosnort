...
print_status "Installing base packages: git libdumbnet-dev ethtool build-essential libpcap0.8-dev libpcre3-dev bison flex autoconf libtool libmysqlclient-dev libnetfilter-queue-dev libnetfilter-queue1 libnfnetlink-dev libnfnetlink0 libarchive-tar-perl libcrypt-ssleay-perl libwww-perl.."

declare -a packages=( git libdumbnet-dev ethtool build-essential libpcap0.8-dev libpcre3-dev bison flex autoconf libtool libmysqlclient-dev libnetfilter-queue-dev libnetfilter-queue1 libnfnetlink-dev libnfnetlink0 libarchive-tar-perl libcrypt-ssleay-perl libwww-perl );
...
case $ui_choice in
	...
	6)
	print_status "Installing sguil.."
	bash autosguil-ubuntu.sh
	print_status "Running sguil installer.."
	error_check 'Sguil interface installation'
	print_notification "sguil installed to: /opt/sguil"
	print_notification "in order to get events you need to start snort_agent, sguild and barnyard2."
	print_notification "You will also need a sguil client to connect to sguild on this sensor on port 7736."
	print_notification "start the sensor: /opt/sguil/sensor/snort_agent.tcl -c /opt/sguil/sensor/snort_agent.conf"
	print_notification "start the server: /opt/sguil/server/sguild -c /opt/sguil/server/sguild.conf -C /opt/sguil/ssl"
	print_notification "If the sensor reboots, or if sguild/snort_agent.tcl fail, you will need to manually restart them. (persistence is on the TODO list)"
	;;
